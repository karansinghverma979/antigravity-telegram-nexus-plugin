...
- **Zero Token Commits**: Tokens are quarantined in the centralized vault at `~/.gemini/credentials/telegram-nexus/config.json` (with legacy fallback to `~/.gemini/config/telegram_config.json`), completely outside all git repositories.
